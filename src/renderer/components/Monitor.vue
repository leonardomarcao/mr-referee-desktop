            ...
            <div class="container-fluid">
              ...
              <b-card v-for="parliamentary in parliamentarians">
                <b-row align-v="center">
                  ...
                  <b-col>
                    <h4>{{ parliamentary.name }}</h4>
                    <p>CPF: {{ parliamentary.document }}</p>
                    <p>Partido: {{ parliamentary.party }}</p>
                    <p>Estado: {{ parliamentary.estate }}</p>
                    <b-checkbox
                        v-model="parliamentary.has_suspicions"
                        value="true"
                        unchecked-value="false"
                        switch
                        @change="onChange(parliamentary.id, $event)">
                      Considerar gastos suspeitos?
                    </b-checkbox>
                  </b-col>
                  <b-col md="auto">
...
export default {
  ...
  data () {
    return {
      loading: false,
      fields: ['Nome', 'Documentação', 'Considera isso suspeito?', 'Partido', 'Estado'],
      parliamentarians: [],
  ...
  methods: {
    ...
    onChange (id, evt) {
      this.updateParliamentary(id, evt)
    },
    async pullParliamentarias () {
      this.loading = true
      await axios.get('http://localhost:3333/get_all_parlamentars', {
        headers: {
          'Content-Type': 'application/json'
        }
      }).then(response => (this.parliamentarians = response.data.map(e => ({
        id: e.id,
        name: e.name,
        document: e.document,
        has_suspicions: e.has_suspicions,
        party: e.party,
        estate: e.estate,
